...
let print_inst inst = match inst with
...
let rec print_prg prg = match prg with 
| hd::tl -> (Printf.fprintf stdout "\n%s" (print_inst hd)); print_prg tl 
| _ -> Printf.fprintf stdout "\n%s" "ENDENDENDEND"
...
| instr :: code' ->
  let env, asm = match instr with
    ...
    | CALL(f, n, p) -> 
                       (* Printf.fprintf stdout "%s" (string_of_int n); *)
                       let rec to_stack n code env = match n with 
                       | 0 -> code, env
                       ...
                       let code = code @ [Call ("L" ^ f); Binop("+", L(n * word_size), esp)] in
                       let code = List.fold_left (fun acc r -> acc @ [Pop r]) code regs in
                       if (p) then env, code else let s, env = env#allocate in env, code @ [Mov(eax, s)]
